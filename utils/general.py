...
VERBOSE = str(os.getenv('', True)).lower() == 'true'  # global verbose mode
...
LOGGER = set_logging('')  # define globally (used in train.py, val.py, detect.py, etc.)


def user_config_dir(dir='Ultralytics', env_var=''):
...
def check_git_status():
    ...
    if n > 0:
        s += f"model is out of date by {n} commit{'s' * (n > 1)}. Use `git pull` or `git clone {url}` to update."
    else:
